...
public interface TendencyAnalysisRepository extends JpaRepository<TendencyAnalysis, Long> {

    // TendencyAnalysis 엔티티에 대한 CRUD 메서드는 JpaRepository에서 제공됨
    // 추가적인 메서드가 필요하면 여기에 정의할 수 있음

    // 가장 최신 성향 분석 결과 가져오기 (userId 기준)
    Optional<TendencyAnalysis> findTopByUserIdOrderByCreatedAtDesc(Long userId);
